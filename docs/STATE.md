...
**NEXT — `docs/HANDOFF-POST-IDENTITY-FOLLOWUPS.md`.** Session 1 code is DONE (#181/#182 merged); **only device-QA remains** — reinstall **main**'s build on Sean's phone (still has the old pre-identity build) and run Part C's LAN checklist (verified-LAN connect + `/shell`, rogue-responder fallback, off-LAN/cellular tunnel fallback, DEBUG self-test). Also covers the pending PR #175 Wave-1 touch-input QA. **Needs Sean's unlocked phone.** Session 2 (later): Terminal UX Wave 2 (selection+copy) — **requires an xterm.js touch-selection research scout first** (`project_terminal_ux_phase`).
