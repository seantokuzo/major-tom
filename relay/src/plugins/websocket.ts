...
const websocketPluginImpl: FastifyPluginAsync = async (fastify) => {
  await fastify.register(websocket, {
    options: {
      clientTracking: true,
      // Phase 13 "The Shell": ring-buffer replay on reattach + `cat` on
      // large files + vimdiff can burst frames well over 1 MiB. 8 MiB
      // gives headroom without making abuse any cheaper (the route still
      // owns throttling).
